...
const SupplierModal: React.FC<SupplierModalProps> = ({ isOpen, supplier, onClose, onSave }) => {
  const [formData, setFormData] = useState(supplier || defaultSupplierState);

  useEffect(() => {
    setFormData(supplier || defaultSupplierState);
  ...
  const modalTitle = supplier ? 'Edit Supplier' : 'Add New Supplier';
        ...
        <div className="p-8 space-y-4">
            <div>
             ...
             <div>
                ...
                <input id="contactPerson" name="contactPerson" type="text" value={formData.contactPerson} onChange={handleChange} className="w-full bg-dark-700 text-white rounded-md border border-dark-600 p-2 focus:ring-2 focus:ring-brand-primary focus:outline-none" />
            </div>
