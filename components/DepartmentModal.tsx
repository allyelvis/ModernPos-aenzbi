...
const defaultDepartmentState: Omit<Department, 'id'> = {
  name: '',
  description: '',
};
...
const DepartmentModal: React.FC<DepartmentModalProps> = ({ isOpen, department, onClose, onSave }) => {
  const [formData, setFormData] = useState(department || defaultDepartmentState);

  useEffect(() => {
    setFormData(department || defaultDepartmentState);
  ...
  const modalTitle = department ? 'Edit Department' : 'Add New Department';
    ...
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50">
      <div className="bg-dark-800 rounded-lg shadow-xl w-full max-w-lg m-4 transform transition-all duration-300 scale-95 animate-scale-in">
        ...
        <div className="p-8 space-y-4">
          <div>
              <label htmlFor="name" className="block text-sm font-medium text-gray-300 mb-1">Department Name</label>
